...
const char *embfile1 = 0;
const char *embfile2 = 0;
...
public:
  ...
  double test(const int imin, const int imax, double *x, const yvec_t &y, const int dims, double *emb_mat1, double *emb_mat2, const int emb_dim1, const int emb_dim2, const double *att_mat1, const double *att_mat2, const char *prefix = "");
	...
	void getXproject(double* emb_mat, double *x, int dim, int emb_dim,double *xproj);
private:
  int nclass;
  int dim1;
	int dim2;
...
void SvmSgdSJE::getXproject(double* emb_mat, double *x, int dim, int emb_dim, double* xproj){
		 ...
		 int emb_dim2, const char *prefix)
{
	...
	double *xproj1 = new double[emb_dim1];
	double *xproj2 = new double[emb_dim2];
	//int i = 0;
	for(int i = imin; i <= imax; i++)
	{
		...
		getXproject(emb_mat1, &xp[dims*i], dims, emb_dim1,xproj1);	
		getXproject(emb_mat2, &xp[dims*i], dims, emb_dim2, xproj2);	
		...
		if(best_index != int(yp.at(i)-1) && best_index != -1)
		{
			...
			for(int iy = 0; iy < emb_dim1; iy++)
			{
				...
				{
					emb_mat1[dims*iy + ix] += eta * xp[dims*i + ix] * (att_mat1[emb_dim1*ni + iy] 
												- att_mat1[emb_dim1*best_index + iy]); 
			...
			for(int iy = 0; iy < emb_dim2; iy++)
			{
				for(int ix = 0; ix < dims; ix++)
				{
					emb_mat2[dims*iy + ix] += eta * xp[dims*i + ix] * (att_mat2[emb_dim2*ni + iy] 
												- att_mat2[emb_dim2*best_index + iy]); 
			//	cout << ix << "," << iy << "=" << emb_mat[dims*iy + ix] << endl;		
				}
			}
		}
  ...
  for(int i = 0; i < nsamples; i++)
  {
  	...
  	for(int i = 0; i < nsamples; i++)
  	{
			...
			if(predicted_class == -1)
				cout << "Warning: max score is below -1" << endl;
...
int main(int argc, const char **argv)
{
	...
	for(int i = 0; i < epochs; i++)
  {
		//timer.start();
		cout << "Epoch " << i << endl; 
  	svm_ale_mult_2.train(imin, imax, nsamples, xtrain, ytrain, dims, att1_mat_train, att2_mat_train, cls_dim_train, emb_mat1, emb_mat2, emb_dim1, emb_dim2);
	...
	if(isval){
		...
		save_embedding_matrix(embfile1, emb_dim1, dims, emb_mat1);
		save_embedding_matrix(embfile2, emb_dim2, dims, emb_mat2);
		
		svm_ale_mult_2_test.test(tmin, tmax, xtest, ytest, dims, emb_mat1, emb_mat2, emb_dim1, emb_dim2, att1_mat_test, att2_mat_test, "test");
	}
