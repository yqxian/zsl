...
public:
  ...
  ~SvmSgdSJE();
public:
  void train(int imin, int imax, int ite_per_epoch, double *xp, const yvec_t &yp, int dims, double *att_mat1, double *att_mat2,  int cls_dim, double* emb_mat1, double *emb_mat2, int emb_dim1, int emb_dim2, double* emb_tensor, const char *prefix = "");
  double test(const int imin, const int imax, double *x, const yvec_t &y, const int dims, double *emb_mat1, double *emb_mat2, const int emb_dim1, const int emb_dim2, const double *att_mat1, const double *att_mat2, double* emb_tensor, const char *prefix = "");
...
void SvmSgdSJE::train(int imin, int imax, int ite_max, double *xp, const yvec_t &yp, int dims, 
		double *att_mat1, double *att_mat2, int cls_dim,  double* emb_mat1, double *emb_mat2, int emb_dim1, 
		 int emb_dim2, double* emb_tensor, const char *prefix)
{
  ...
  double *A = new double[emb_dim1*emb_dim2];
  double *B = new double[emb_dim1*emb_dim2];
	//int i = 0;
	for(int i = imin; i <= imax; i++)
	{
		for(int j = 0; j < emb_dim1; j++)
    	for(int k = 0; k < emb_dim2; k++)
        A[emb_dim2*j + k] = 0.0;
    int saver = emb_dim2 * dims;
    //compute matrix A to eliminate the loop over dim of xp
    for(int j = 0; j < emb_dim1; j++)
      for(int k = 0; k < emb_dim2; k++)
        for(int ix = 0; ix < dims; ix++)
          A[emb_dim2*j + k] += xp[dims*i + ix] * emb_tensor[saver*j + dims*k + ix];
		//i = rand()%imax + imin;
        ...
        //loop over output embeddiing y2
        for(int iy2 = 0; iy2 < emb_dim2; iy2++){
            score += A[emb_dim2*iy1 + iy2] * att_mat1[emb_dim1*j + iy1] * att_mat2[j*emb_dim2 + iy2];
          }
      }
		...
		if(best_index != int(yp.at(i)-1) && best_index != -1)
		{
			...
			for(int iy1 = 0; iy1 < emb_dim1; iy1++)
        for(int iy2 = 0; iy2 < emb_dim2; iy2++)
            A[iy1*emb_dim2 + iy2] = att_mat1[ni*emb_dim1 + iy1] * att_mat2[ni*emb_dim2 + iy2];
  //    cout << "finish computing A" << endl;
      for(int iy1 = 0; iy1 < emb_dim1; iy1++)
      	for(int iy2 = 0; iy2 < emb_dim2; iy2++)
        	B[iy1*emb_dim2 + iy2] = att_mat1[best_index*emb_dim1 + iy1] * att_mat2[best_index*emb_dim2 + iy2];
  //    cout << "Finish computing Matrix A and B" << endl;
      //loop over output embedding y1
      for(int iy1 = 0; iy1 < emb_dim1; iy1++){
        //loop over output embeddiing y2
        for(int iy2 = 0; iy2 < emb_dim2; iy2++){
          for(int ix = 0; ix < dims; ix++){
            emb_tensor[saver*iy1 + dims*iy2 + ix] -= eta * xp[dims*i+ix] * (B[iy1*emb_dim2+iy2] - A[iy1*emb_dim2+iy2]);
          }
        }
      }
			for(int iy = 0; iy < emb_dim1; iy++)
			...
			for(int iy = 0; iy < emb_dim2; iy++)
			{
				for(int ix = 0; ix < dims; ix++)
				{
					emb_mat2[dims*iy + ix] -= eta * xp[dims*i + ix] * (att_mat2[emb_dim2*best_index + iy] 
												- att_mat2[emb_dim2*ni + iy]); 
			//	cout << ix << "," << iy << "=" << emb_mat[dims*iy + dsafx] << endl;		
				}
			}
		}
		t += 1;
...
double SvmSgdSJE::test(const int imin, const int imax, double *xp, const yvec_t &yp, const int dims,  
		double *emb_mat1, double *emb_mat2, const int emb_dim1, const int emb_dim2, const double *att_mat1, const double *att_mat2, double* emb_tensor, const char* prefix)
{
  ...
  for(int i = 0; i < nsamples; i++)
  {
    ...
    for(int j = 0; j < emb_dim1; j++)
      for(int k = 0; k < emb_dim2; k++)
        for(int ix = 0; ix < dims; ix++)
          A[j*emb_dim2 + k] += xp[dims*i+ix] * emb_tensor[saver*j + dims*k + ix];
		...
		for(int j = 0; j < nclass; j++)
  	{
        ...
        //loop over output embeddiing y2
        for(int iy2 = 0; iy2 < emb_dim2; iy2++){
          scores[nsamples*j+i] += A[iy1*emb_dim2+iy2] * att_mat1[j*emb_dim1+iy1] * att_mat2[j*emb_dim2+iy2];
        }
      }
			for(int iy = 0; iy < emb_dim1; iy++)
...
int main(int argc, const char **argv)
{
  ...
  cout << "The size of the embedding matrix2 is " << emb_dim2 << "*"  << dims << " " << endl;
	...
	emb_tensor = new double[emb_dim1*emb_dim2*dims];
	...
	std_dev = 1.0 / sqrt(emb_dim2*dims);
  int saver = dims*emb_dim2;
  double t_v;
  for (int e1 = 0; e1 < emb_dim1; e1++)
  {
    for(int e2 = 0; e2 < emb_dim2; e2++){
      ...
      {
        t_v = std_dev * rand_gen();
        //squaresum += t_v * t_v;
        emb_tensor[saver*e1+dims*e2+i] = t_v;
        //emb_tensor_fvec[e1][e2]->set(i, 0);
      }
      //for(int i = 0; i < dims; i){
      //  emb_tensor[saver*e1 + dims*e2 + i] *= lambda/sqrt(squaresum);
    //  }
    }
  }
  ...
  if(testfile)
	...
	double best_emb_mat1[emb_dim1*dims];
	double best_emb_mat2[emb_dim2*dims];
	double best_emb_tensor[emb_dim1*emb_dim2*dims];
	for(int i = 0; i < epochs_first; i++)
  {
		//timer.start();
		cout << "Epoch " << i << endl; 
  	svm_ale_mult_2.train(imin, imax, ite_per_epoch, xtrain, ytrain, dims, att1_mat_train, att2_mat_train, cls_dim_train, emb_mat1, emb_mat2, emb_dim1, emb_dim2, emb_tensor);
		//svm_ale_mult_2.test(imin, imax,  xtrain, ytrain, dims, emb_mat, emb_dim, att_mat_train, "train");
		if(isval){
			cur_accuracy = svm_ale_mult_2_test.test(tmin, tmax, xtest, ytest, dims, emb_mat1, emb_mat2, emb_dim1, emb_dim2, att1_mat_test, att2_mat_test, emb_tensor, "test");
			if(cur_accuracy > best_accuracy){
				best_accuracy = cur_accuracy;
				best_nepoch = i + 1; 
				memcpy(best_emb_mat1, emb_mat1, emb_dim1*dims*sizeof(double));
				memcpy(best_emb_mat2, emb_mat2, emb_dim2*dims*sizeof(double));
				memcpy(best_emb_tensor, emb_tensor, emb_dim1*emb_dim2*dims*sizeof(double));
			}
		}	
	}
	if(isval){
		cout << "Per class accuracy=" << best_accuracy << " ,eta=" << eta << " ,nepoch=" << best_nepoch <<endl; 
		cout << cur_accuracy << " " << eta << " "  << best_nepoch << endl;
	}
	else{
		save_embedding_matrix(embmatfile1, emb_dim1, dims, emb_mat1);
    save_embedding_matrix(embmatfile2, emb_dim2, dims, emb_mat2);	
		save_emb_tensor(embtensorfile, emb_dim1, emb_dim2, dims, emb_tensor);
		
		svm_ale_mult_2_test.test(tmin, tmax, xtest, ytest, dims, emb_mat1, emb_mat2, emb_dim1, emb_dim2, att1_mat_test, att2_mat_test, emb_tensor, "test");
	}	
	delete emb_mat1;
	delete emb_mat2;
	delete emb_tensor;
