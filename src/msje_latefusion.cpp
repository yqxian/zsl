...
public:
  ...
  ~SvmSgdSJE();
public:
  double test(double* score_mat1, double* score_mat2, double* score_tensor, const yvec_t &yp, int nsamples, int nclass, double alpha, double beta);

  void getScore(const int imin, const int imax, double *x,  const int dims, double *emb_mat1, double *emb_mat2, const int emb_dim1, const int emb_dim2, const double *att_mat1, const double *att_mat2, double* emb_tensor, double* score_mat1, double* score_mat2, double* score_tensor);
...
void SvmSgdSJE::getScore(const int imin, const int imax, double *xp, const int dims,  
		double *emb_mat1, double *emb_mat2, const int emb_dim1, const int emb_dim2, const double *att_mat1, const double *att_mat2, double* emb_tensor, double *score_mat1, double *score_mat2, double *score_tensor)
{
    ...
    for(int j = 0; j < emb_dim1; j++)
		...
		for(int j = 0; j < nclass; j++)
  	{
        ...
        for(int iy2 = 0; iy2 < emb_dim2; iy2++){
          score_tensor[nsamples*j+i] += A[iy1*emb_dim2+iy2] * att_mat1[j*emb_dim1+iy1] * att_mat2[j*emb_dim2+iy2];
        }
      }
...
/// Testing
double SvmSgdSJE::test(double *score_mat1, double *score_mat2, double *score_tensor, const yvec_t &yp,  
													int nsamples, int nclass, double alpha, double beta)
{

	double delta = 1 - alpha - beta;
  ...
  for(int i = 0; i < nsamples; i++)
  {
		...
		{
			double score = delta*score_tensor[nsamples*c+i] + alpha*score_mat1[nsamples*c+i] + beta*score_mat2[nsamples*c+i]; 
			if ( score > max_score )
			{
				predicted_class = c;
				max_score = score;
  	...
  	cout << "delta=" << delta << "alpha=" << alpha << "beta=" << beta << ", Per " << " class accuracy = " << setprecision(4) << 100 * double(sum_diag_conf / nclass) << "%." << endl;
...
int main(int argc, const char **argv)
{
 ...
 // Load test images and their labels 
  ...
  SvmSgdSJE svm_ale_mult_2_test(cls_dim_test, emb_dim1, emb_dim2, lambda, eta); 
	...
	load_emb_tensor(embtensorfile, emb_dim1, emb_dim2, dims, emb_tensor);

  double *score_mat1 = new double[nclass*nsamples];
  double *score_mat2 = new double[nclass*nsamples];
  double *score_tensor = new double[nclass*nsamples];
	svm_ale_mult_2_test.getScore(tmin, tmax, xtest, dims, emb_mat1, emb_mat2, emb_dim1, emb_dim2, att1_mat_test, att2_mat_test, emb_tensor, score_mat1, score_mat2, score_tensor);
	...
	if(isval){
		...
		for(int i = 0; i < 300; i++){
			alpha = frand(0.0, 1.0);
			beta = frand(0.0, 1.0 - alpha);	
			cur_accuracy = svm_ale_mult_2_test.test(score_mat1, score_mat2, score_tensor, ytest, xtest_fvec.size(), cls_dim_test, alpha, beta);
		...
		cout << best_accuracy << " " << best_alpha << " " << best_beta << endl;
	}
	else{
		svm_ale_mult_2_test.test(score_mat1, score_mat2, score_tensor, ytest, xtest_fvec.size(), cls_dim_test, alpha, beta);
	}
  return 0;
}
