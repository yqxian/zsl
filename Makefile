...
BIN=$(PATH_ALL)/sje_latefusion/bin
TMP=$(PATH_ALL)/sje_latefusion/results_$(DATASET)
DATA=$(PATH_ALL)/data/$(DATASET)
# INT = wherever the learned embeddings are going to be saved
INT=$(DATA)/SJE_LATEFUSION
...
run_allcombine2:
	screen -S sje_latefusion_${DATASET}_$(EMB5)$(EMB2) -m -d make run_batch ATT1=$(EMB5) ATT2=$(EMB2)
	screen -S sje_latefusion_${DATASET}_$(EMB5)$(EMB3) -m -d make run_batch ATT1=$(EMB5) ATT2=$(EMB3)
	screen -S sje_latefusion_${DATASET}_$(EMB5)$(EMB4) -m -d make run_batch ATT1=$(EMB5) ATT2=$(EMB4)
...
run_val: 
	make $(TMP)/sje_latefusion_${ATT1}${ATT2}_val.txt

$(TMP)/sje_latefusion_${ATT1}${ATT2}_val.txt:
	$(BIN)/sje_latefusion -val\
		...
		$(INT)/emb_mat_${ATT1}_val $(INT)/emb_mat_${ATT2}_val $(DATA)/xtrain_mean $(DATA)/xtrain_variance $(DATA)/xtrain_max \
	>> $@

run_one_test:
	make $(TMP)/sje_latefusion_${ATT1}${ATT2}_test.txt

$(TMP)/sje_latefusion_${ATT1}${ATT2}_test.txt:
	$(BIN)/sje_latefusion -alpha ${ALPHA}\
		...
		$(INT)/emb_mat_${ATT1}_test $(INT)/emb_mat_${ATT2}_test $(DATA)/xtrainval_mean $(DATA)/xtrainval_variance $(DATA)/xtrainval_max \
	>> $@
...
run_test:
	...
	less $(TMP)/sje_latefusion_${ATT1}${ATT2}_val.txt | tail -1 > $(TMP)/param_${ATT1}${ATT2}.txt
	make run_one_test ALPHA=`awk '{print $$2}' $(TMP)/param_${ATT1}${ATT2}.txt`
